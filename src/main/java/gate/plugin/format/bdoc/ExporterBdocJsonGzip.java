 ...
 */
@CreoleResource(
        name = "Bdoc/JSON Gzip Exporter", 
        tool = true, 
        autoinstances = @AutoInstance, 
        comment = "Export GATE documents in Bdoc/JSON Gzip format.", 
        helpURL = "https://github.com/GateNLP/gateplugin-Format_Bdoc"
)
public class ExporterBdocJsonGzip extends DocumentExporter {

  private static final long serialVersionUID = 776942354560368L;

  /**
   * Constructor.
   */
  public ExporterBdocJsonGzip() {    
    super("BdocJson/Gzip", "bdocjs.gz", "text/bdocjs+gzip");
  }
  ...
  public void export(Document dcmnt, OutputStream out, FeatureMap fm) throws IOException {
    ...
    new Saver().to(out).format(Format.JSON_MAP).gzipped(true).save(bdoc);
  }
